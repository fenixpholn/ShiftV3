...
class DutyScheduler:
    ...
    def generate_schedule(self, num_days):
        ...
        duty_counts = {doc: {"total": 0, "holiday": 0} for doc in self.doctors}
        ...
        for day in range(1, num_days+1):
            ...
            for role in self.roles:
                valid_doctors = [doc for doc in available_doctors if role not in self.constraints[doc]["unavailable_roles"]
                                 and duty_counts[doc]["total"] < self.constraints[doc]["max_shifts"]
                                 and doc not in assigned_doctors]
                
                if valid_doctors:
                    selected_doctor = min(valid_doctors, key=lambda d: (duty_counts[d]["total"], duty_counts[d]["holiday"] if is_holiday else 0))
                    self.schedule.at[day, role] = selected_doctor
                    duty_counts[selected_doctor]["total"] += 1
                    if is_holiday:
                        duty_counts[selected_doctor]["holiday"] += 1
...
for doctor in scheduler.doctors:
    if doctor in scheduler.constraints:
        with st.expander(f"{doctor}: สูงสุด {scheduler.constraints[doctor]['max_shifts']} เวร | ไม่สะดวก: {scheduler.constraints[doctor]['unavailable_days']} | เวรที่ไม่สะดวก: {scheduler.constraints[doctor]['unavailable_roles']}"):
